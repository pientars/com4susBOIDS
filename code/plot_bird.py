...
ts = np.genfromtxt('bird_timeseries.csv', delimiter=',');
#clevs = [0,0.02,0.04,0.06,0.08,0.1,0.12,0.14,0.16,0.18,0.2,0.22,0.24,0.26,0.28,0.3,1]
tsdata = ts[:,window].T;

min_lon = lat.min(axis=0); max_lat = lat.max(axis=0);
min_lon = lon.min(axis=0); max_lon = lon.max(axis=0);
xi = np.linspace(min_lon,max_lon,50)
yi = np.linspace(min_lat,max_lat,50)
lon2, lat2 = np.meshgrid(xi,yi)
xo, yo = m(lon2, lat2)
#zi = griddata(x,y,tsdata,xi,yi,interp='nn')
vals = tsdata.reshape([50,50]);
cs = m.contourf(xo,yo,vals,cmap=plt.cm.rainbow)
